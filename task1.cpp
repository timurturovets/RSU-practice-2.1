...
public:
    ...
    int encode(char const * input, char const * output, bool should_decrypt) {
        ...
        if (!ofs.is_open()) throw std::ofstream::failure("Couldnt' encode: unable to open output file.");

        ifs.seekg(0, std::ios::end);
        std::streamsize len = ifs.tellg();
        ifs.seekg(0, std::ios::beg);

        auto *buf = (unsigned char*) malloc(len * sizeof(char));
